...
async function uploadSecondCourse(dryRun: boolean = true) {
  console.log('🚀 Starting upload process for second course...\n');

  // Read migration data
  const dataPath = path.join(__dirname, '../final-migration-data-backup.json');
  const migrationData: MigrationData = JSON.parse(fs.readFileSync(dataPath, 'utf-8'));

  // Get second course (index 1)
  const secondCourse = migrationData.cursos[1];
  console.log(`📚 Second Course: ${secondCourse.title} (ID: ${secondCourse.id})`);
  console.log(`   Description: ${secondCourse.description}`);
  console.log(`   Modules: ${secondCourse.modules?.length || 0}`);

  // Get modules for this course
  const courseModules = migrationData.modules.filter(m => m.courseId === secondCourse.id);
  ...
  const courseContent = migrationData.content.filter(c => c.courseId === secondCourse.id);
  ...
  if (dryRun) {
    ...
    console.log('\nSummary:');
    console.log(`- Course: 1 document (${secondCourse.id})`);
  ...
  console.log(`📤 Uploading course: ${secondCourse.title}...`);
  const { id: courseId, ...courseDataRaw } = secondCourse;
  ...
  for (const content of courseContent) {
    const { id: contentId, description, url, questions, ...contentDataRaw } = content;
    // Map fields to correct structure and sanitize
    const contentData = sanitizeData({
      ...contentDataRaw,
      videoUrl: url, // Map 'url' to 'videoUrl'
      order: uploadCount + 1,
      status: 'Activo'
    });
    try {
