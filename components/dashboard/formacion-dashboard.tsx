    ...
    <div className="space-y-6">
      ...
      <div className="grid gap-6 grid-cols-1 md:grid-cols-2 xl:grid-cols-3 mt-8">
        ...
        <RechartCard
          ...
          colors={COLORS}
        />
        {/* ...repites para maestros, horario, nivel, modalidad */}
      </div>
          ...
          <CardContent>
            <div className="space-y-4">
              {EstudiantesPendientes.map((student) => (
                <div key={student.id} className="flex items-center gap-4 rounded-md border p-4">
                  <Avatar>
                  ...
                  {student.status === "urgent" && <AlertTriangle className="h-4 w-4 text-amber-500" />}
                </div>
              ))}
            </div>
          </CardContent>
